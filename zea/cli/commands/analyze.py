"""zea analyze — full pipeline: Discovery → Graph → Inference."""
...
from zea.core.logging import setup_logging
from zea.discovery.scanner import save_inventory, scan_repository
...
def analyze_command(
    ...
    output_dir: Path = typer.Option(Path(".zea"), "--output", "-o", help="Output directory"),
    verbose: bool = typer.Option(False, "--verbose", "-v"),
    skip_inference: bool = typer.Option(False, "--no-inference", help="Skip architecture inference"),
) -> None:
    ...
    console.print(f"\n[bold blue]ZEA[/bold blue] Analyzing [cyan]{repo_path.resolve().name}[/cyan]\n")
...
def _print_inventory_summary(inventory: object) -> None:
    from zea.discovery.models import RepositoryInventory
